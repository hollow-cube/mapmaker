...
public sealed interface HubPlayerState extends PlayerState<HubPlayerState, HubMapWorld> {
    ...
    record Default() implements HubPlayerState {
        private static final AttributeModifier REACH_MOD = new AttributeModifier(
            "mapmaker:hub_reach",
            40 - Attribute.ENTITY_INTERACTION_RANGE.defaultValue(),
            AttributeOperation.ADD_VALUE
        );

        private static final List<BossBar> BOSS_BARS = List.of(
            BossBars.createLine1(
                Component.text(
                    FontUtil.rewrite("bossbar_ascii_1", "Map Maker Early Access"),
                    TextColor.color(0x3895FF)
                )
            ),
        ...
        public void configurePlayer(
            HubMapWorld world,
            Player player,
            @Nullable HubPlayerState lastState
        ) {
            ...
            player.setHeldItemSlot(
                playerData.getSetting(PlayerSettings.HUB_SELECTED_SLOT).byteValue()
            );
        ...
        public void resetPlayer(
            HubMapWorld world,
            Player player,
            @Nullable HubPlayerState nextState
        ) {
            ...
            FutureUtil.submitVirtual(
                () -> playerData.writeUpdatesUpstream(world.server().playerService())
            );
        }
    }
