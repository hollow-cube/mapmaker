package net.hollowcube.mapmaker.runtime.parkour.action.gui.editors.attributes;
...
public class AttributeEditor extends AbstractActionEditorPanel<EditAttributeAction> {
    ...
    public AttributeEditor(ActionList.Ref ref, ActionAttributes.Entry entry) {
        super(ref);
    ...
    @Override
    protected void update(EditAttributeAction data) {
        ...
        switch (data.operation()) {
            ...
            case ADD, SUBTRACT ->
                    this.value.range(0.0, Math.abs(data.attribute().minValue()) + Math.abs(data.attribute().maxValue()));
        }
    }
}
