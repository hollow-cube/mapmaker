...
public class MapLeaderboardMarkerHandler extends ObjectEntityHandler {
    ...
    private @Nullable LeaderboardDisplay leaderboard;
    private int ticksUntilUpdate = ONE_MINUTE_TICKS;

    public MapLeaderboardMarkerHandler(MarkerEntity entity) {
        super(ID, entity);

        var world = MapWorld.forInstance(entity.getInstance());
        if (world == null) throw new RuntimeException("missing world");

        if (!world.map().isPublished() || world.canEdit(null)) {
