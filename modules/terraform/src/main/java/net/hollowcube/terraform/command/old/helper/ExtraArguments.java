...
public final class ExtraArguments {
    ...
    public static @NotNull Argument<@NotNull Clipboard> Clipboard(@NotNull String id) {
        return ArgumentType.String(id)
                .map((sender, input) -> {
                    if (!(sender instanceof Player player)) {
                        throw new IllegalStateException("Only players can have clipboards");
                    }

                    var session = PlayerSession.forPlayer(player);
                    try {
                        return session.clipboard(input);
                    } catch (IllegalArgumentException e) {
                        throw new ArgumentSyntaxException(e.getMessage(), input, 0);
                    }
                })
                .setSuggestionCallback((sender, context, suggestion) -> {
                    if (!(sender instanceof Player player)) {
                    ...
                    var session = PlayerSession.forPlayer(player);
                    for (var clipboardName : session.clipboardNames()) {
                        //todo add a hover giving some information about the clipboards.
                        suggestion.addEntry(new SuggestionEntry(clipboardName));
                    }
                });
    }
