...
public class V2514 extends DataVersion {
    ...
    private static @Nullable Value updateProjectile(Value entity) {
        entity.put("Owner", entity.remove("OwnerUUID"));
        return null;
    }

    private static @Nullable Value updateHurtBy(Value entity) {
        return UUIDFixes.replaceUuidFromLeastMost(entity, "HurtBy", "HurtBy");
    }

    private static @Nullable Value updateFox(Value entity) {
    ...
    private static @Nullable Value updateItem(Value entity) {
        var _ = UUIDFixes.replaceUuidFromMLTag(entity, "Owner", "Owner");
        return UUIDFixes.replaceUuidFromMLTag(entity, "Thrower", "Thrower");
    }

    private static @Nullable Value updateShulkerBullet(Value entity) {
        var _ = UUIDFixes.replaceUuidFromMLTag(entity, "Owner", "Owner");
        return UUIDFixes.replaceUuidFromMLTag(entity, "Target", "Target");
    }

    private static @Nullable Value updateAreaEffectCloud(Value entity) {
        return UUIDFixes.replaceUuidFromMLTag(entity, "OwnerUUID", "Owner");
    }

    private static @Nullable Value updateZombieVillager(Value entity) {
        return UUIDFixes.replaceUuidFromMLTag(entity, "ConversionPlayer", "ConversionPlayer");
    }

    private static @Nullable Value updateEvokerFangs(Value entity) {
        return UUIDFixes.replaceUuidFromMLTag(entity, "OwnerUUID", "Owner");
    }

    private static @Nullable Value updatePiglin(Value entity) {
    ...
    private static @Nullable Value updateConduitBlockEntity(Value blockEntity) {
        return UUIDFixes.replaceUuidFromMLTag(blockEntity, "target_uuid", "Target");
    }

    private static @Nullable Value updateSkullBlockEntity(Value blockEntity) {
        UUIDFixes.replaceUuidFromString(blockEntity.get("Owner"), "Id", "Id");
        blockEntity.put("SkullOwner", blockEntity.remove("Owner"));
        return null;
    }

    private static @Nullable Value updateItemAttributeModifiers(Value blockEntity) {
    ...
    private static @Nullable Value updateSkullItem(Value blockEntity) {
        var tag = blockEntity.get("tag");
        if (!tag.isMapLike()) return null;
