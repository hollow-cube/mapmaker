...
import net.hollowcube.mapmaker.hub.gui.HubUIs;
...
public class HubServerOld implements HubManager {
    ...
    private void handleUseItem(@NotNull PlayerUseItemEvent event) {
        ...
        switch (item.meta().getCustomModelData()) {
            case ItemUtils.PLAY_ITEM_CMD -> player.openInventory(new RouterSection(new BuildMaps()).getInventory());
            case ItemUtils.CREATE_ITEM_CMD -> new RouterSection(HubUIs.createMaps()).showToPlayer(player);
        }
