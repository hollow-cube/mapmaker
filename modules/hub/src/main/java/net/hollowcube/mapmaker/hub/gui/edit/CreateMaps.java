package net.hollowcube.mapmaker.hub.gui.edit;

import net.hollowcube.canvas.Label;
import net.hollowcube.canvas.View;
import net.hollowcube.canvas.annotation.Outlet;
import net.hollowcube.mapmaker.model.PlayerData;
import net.minestom.server.entity.Player;
import org.jetbrains.annotations.NotNull;

public class CreateMaps extends View {

    private @Outlet("slot0") Label slot0;
    private @Outlet("slot1") Label slot1;
    private @Outlet("slot2") Label slot2;
    private @Outlet("slot3") Label slot3;
    private @Outlet("slot4") Label slot4;

    private final PlayerData playerData;
    private final Label[] slots;

    public CreateMaps(@NotNull Player player) {
        playerData = PlayerData.fromPlayer(player);
        slots = new Label[]{slot0, slot1, slot2, slot3, slot4};
    }

    @Override
    protected void mount() {
        super.mount();

        for (int i = 0; i < slots.length; i++) {
            var slot = slots[i];

            // If the slot is locked, show the lock icon
            if (i >= playerData.getUnlockedMapSlots()) {
                //todo
                continue;
            }

            var mapId = playerData.getMapSlot(i);
            // If there is no map here, show the empty icon
            if (mapId == null) {
                //todo
                continue;
            }

            // Finally, set to loading, fetch the map info and show it
            slot.setLoading(true);
        }
    }
}
