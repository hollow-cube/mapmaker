...
class Protocol776Test {
    ...
    void testTableSizesMatchTheRegistrationOrder() {
        assertEquals(69, Protocol776.PACKETS.entries(ProtocolState.PLAY, Direction.C2S).size());
        assertEquals(141, Protocol776.PACKETS.entries(ProtocolState.PLAY, Direction.S2C).size());
        assertEquals(10, Protocol776.PACKETS.entries(ProtocolState.CONFIGURATION, Direction.C2S).size());
        assertEquals(20, Protocol776.PACKETS.entries(ProtocolState.CONFIGURATION, Direction.S2C).size());
        assertEquals(1, Protocol776.PACKETS.entries(ProtocolState.HANDSHAKE, Direction.C2S).size());
        assertEquals(0, Protocol776.PACKETS.entries(ProtocolState.HANDSHAKE, Direction.S2C).size());
    }
    ...
    void testKnownPacketIds() {
        ...
        assertEquals(30, Protocol776.PACKETS.packetId(ProtocolState.PLAY, Direction.C2S, "move_player_pos"));
        assertEquals(33, Protocol776.PACKETS.packetId(ProtocolState.PLAY, Direction.C2S, "move_player_status_only"));
        assertEquals(45, Protocol776.PACKETS.packetId(ProtocolState.PLAY, Direction.C2S, "pong"));
        assertEquals(7, Protocol776.PACKETS.packetId(ProtocolState.CONFIGURATION, Direction.S2C, "registry_data"));
        assertEquals(3, Protocol776.PACKETS.packetId(ProtocolState.CONFIGURATION, Direction.C2S, "finish_configuration"));
    }
    ...
    void testDecodedPacketsHaveDecodersAndKeepListPacketsAreKept() {
        for (String name : List.of("level_chunk_with_light", "block_update", "section_blocks_update",
            ...
            var entry = Protocol776.PACKETS.lookup(ProtocolState.PLAY, Direction.S2C, name);
            assertEquals(PacketTable.KeepKind.KEEP_DECODE, entry.kind(), name);
        ...
        for (String name : List.of("move_player_pos", "move_player_pos_rot", "move_player_rot",
            ...
            var entry = Protocol776.PACKETS.lookup(ProtocolState.PLAY, Direction.C2S, name);
            assertEquals(PacketTable.KeepKind.KEEP_DECODE, entry.kind(), name);
            assertNotNull(entry.decoder(), name);
        }
        ...
        for (String name : AUDIT_YES)
            assertTrue(Protocol776.PACKETS.lookup(ProtocolState.PLAY, Direction.S2C, name).kept(), name);
        ...
        for (String name : List.of("accept_teleportation", "attack", "interact", "player_action", "player_command",
            ...
            assertTrue(Protocol776.PACKETS.lookup(ProtocolState.PLAY, Direction.C2S, name).kept(), name);
    }
    ...
    void testAuditNoRowsAreDropped() {
        for (String name : List.of("award_stats", "block_destruction", "boss_event", "clear_titles", "commands",
            ...
            assertFalse(Protocol776.PACKETS.lookup(ProtocolState.PLAY, Direction.S2C, name).kept(), name);
        ...
        for (String name : List.of("chat", "chat_command", "edit_book", "rename_item", "set_beacon", "sign_update",
            ...
            assertFalse(Protocol776.PACKETS.lookup(ProtocolState.PLAY, Direction.C2S, name).kept(), name);
    }
    ...
    void testPingSetIsTheAuditYesRowsMinusEntityAndScreenGroups() {
        ...
        for (PacketTable.Entry entry : Protocol776.PACKETS.entries(ProtocolState.PLAY, Direction.S2C))
    ...
    void testConditionalFencesCoverTheOwnPlayerLeaksOfTheEntityGroup() {
        ...
        assertFence(true, "animate", new S2CAnimate.V776(9, S2CAnimate.V776.WAKE_UP), local);
    ...
    void testPingSetMembersAreAlwaysKept() {
        for (ProtocolState state : ProtocolState.values())
            for (Direction direction : Direction.values())
                for (PacketTable.Entry entry : Protocol776.PACKETS.entries(state, direction))
                    if (entry.pingSet()) assertTrue(entry.kept(), entry.name());
    }

    @Test
    void testConfigurationPhaseHasNoPings() {
        for (PacketTable.Entry entry : Protocol776.PACKETS.entries(ProtocolState.CONFIGURATION, Direction.S2C))
            assertFalse(entry.pingSet(), entry.name());
    }

    @Test
    void testUnknownIdsAndNamesResolveToDrop() {
        assertSame(PacketTable.UNKNOWN, Protocol776.PACKETS.lookup(ProtocolState.PLAY, Direction.S2C, 141));
        assertSame(PacketTable.UNKNOWN, Protocol776.PACKETS.lookup(ProtocolState.PLAY, Direction.S2C, -1));
        assertSame(PacketTable.UNKNOWN, Protocol776.PACKETS.lookup(ProtocolState.PLAY, Direction.S2C, "nonexistent"));
        assertEquals(-1, Protocol776.PACKETS.packetId(ProtocolState.PLAY, Direction.S2C, "nonexistent"));
        assertFalse(PacketTable.UNKNOWN.kept());
        assertNull(PacketTable.UNKNOWN.decoder());
    }
    ...
    void testLookupByIdAndNameAgree() {
        for (var state : ProtocolState.values()) {
            for (var direction : Direction.values()) {
                var entries = Protocol776.PACKETS.entries(state, direction);
                for (int id = 0; id < entries.size(); id++) {
                    var entry = entries.get(id);
                    assertSame(entry, Protocol776.PACKETS.lookup(state, direction, id));
                    assertEquals(id, Protocol776.PACKETS.packetId(state, direction, entry.name()));
                }
            }
        }
    }
    ...
    private static void assertFence(boolean expected, String name, Packet packet, int localPlayerId) {
        var when = Protocol776.PACKETS.lookup(ProtocolState.PLAY, Direction.S2C, name).pingWhen();
