...
public final class MapBuilderAcceptedNotificationType implements PlayerNotificationType {
    ...
    private static EntryData dataFromEntry(Player player, ServiceContext context, String key, JsonObject data) {
        var accepted = Boolean.parseBoolean(key);

        var mapId = data.get("mapId").getAsString();
        var builderId = data.get("builderId").getAsString();
    ...
    public PlayerNotification createNotification(Player player, ServiceContext context, Notification entry) {
        var data = dataFromEntry(player, context, entry.key(), entry.data());
