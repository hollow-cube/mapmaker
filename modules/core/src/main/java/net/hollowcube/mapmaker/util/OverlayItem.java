...
public final class OverlayItem {
    ...
    public static @NotNull ItemStack.Builder with(@NotNull ItemStack.Builder builder, @NotNull Material material, @Nullable String overlay) {
    ...
    public static @NotNull ItemStack with(@NotNull ItemStack stack, @NotNull Material material, @Nullable String overlay) {
        String model = Objects.requireNonNullElse(material.prototype().get(DataComponents.ITEM_MODEL), "paper");
        if (model.startsWith("minecraft:")) {
            model = model.substring("minecraft:".length());
        }
    ...
    private static List<String> getModelData(@NotNull ItemStack stack) {
    ...
    public static @Nullable String getOverlay(@NotNull ItemStack stack) {
        var data = getModelData(stack);
        return data.isEmpty() || data.size() < 2 ? null : data.get(1);
    }

    public static @Nullable String getBaseModel(@NotNull ItemStack stack) {
        var data = getModelData(stack);
