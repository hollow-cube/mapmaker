...
//
//    public PlayerInviteServiceImpl(@NotNull String url, @NotNull MapServer server) {
...
//        var senderSession = server.sessionManager().getSession(sender.getUuid().toString());
...
//        }
//
//        var targetDisplayName = server.playerService().getPlayerDisplayName2(targetId);
//        var targetSession = server.sessionManager().getSession(targetId);
...
//        }
//
//        var targetMap = server.mapService().getMap(targetId, targetPresence.mapId());
...
//        server.bridge().joinMap(sender, targetMap.id(), joinState);
//    }
//
//    @Override
...
//        }
//
//        var targetMap = MiscFunctionality.getCurrentMap(server.sessionManager(), server.mapService(), targetId);
...
//        }
//
//        var senderMap = MiscFunctionality.getCurrentMap(server.sessionManager(), server.mapService(), sender);
...
//                var inviteMap = server.mapService().getMap(invite.senderId(), invite.mapId());
//
...
//                var targetDisplayName = server.playerService().getPlayerDisplayName2(inviteTargetId);
//
...
//        return MiscFunctionality.getCurrentMap(server.sessionManager(), server.mapService(), player);
//    }
//
