...
public class InteractionRules {
    private static final Int2ObjectMap<net.hollowcube.map2.block.interaction
            .BlockInteractionRule> blockRules = new Int2ObjectArrayMap<>();
    private static final Int2ObjectMap<net.hollowcube.map2.block.interaction
            .BlockInteractionRule> itemRules = new Int2ObjectArrayMap<>();
    ...
    private static void handleBlockInteract(@NotNull PlayerBlockInteractEvent event) {
        ...
        if (rule != null && rule.sneakState().test(player.isSneaking(), !itemStack.isAir())) {
            var interaction = new net.hollowcube.map2.block.interaction
                    .BlockInteractionRule.Interaction(
    ...
    private static void handleItemUse(@NotNull PlayerUseItemEvent event) {
        ...
        if (!(rule instanceof net.hollowcube.map2.block.interaction
                .BlockInteractionRule.AirInteractionRule airRule)
    ...
    private static void item(@NotNull Material material, @NotNull net.hollowcube.map2.block.interaction
            .BlockInteractionRule rule) {
        itemRules.put(material.id(), rule);
    }

    private static void item(@NotNull Collection<NamespaceID> tag, @NotNull net.hollowcube.map2.block.interaction
            .BlockInteractionRule rule) {
        tag.forEach(id -> item(Objects.requireNonNull(Material.fromNamespaceId(id)), rule));
    }

    private static void block(@NotNull Block block, @NotNull net.hollowcube.map2.block.interaction
            .BlockInteractionRule rule) {
        blockRules.put(block.id(), rule);
    }

    private static void block(@NotNull Collection<NamespaceID> tag, @NotNull net.hollowcube.map2.block.interaction
            .BlockInteractionRule rule) {
