...
public class RayUtils {
    public static boolean BoundingBoxIntersectionCheck(BoundingBox moving, Point rayStart, Point rayDirection, BoundingBox collidableStatic, Point staticCollidableOffset, SweepResult finalResult, Object obj) {
        ...
        if (percentage >= 0 && percentage <= finalResult.res && percentage <= 1 && isHit) {
    ...
    public static boolean BoundingBoxRayIntersectionCheck(Vec start, Vec direction, BoundingBox boundingBox, Vec position, SweepResult res, Object obj) {
        return BoundingBoxIntersectionCheck(BoundingBox.ZERO, start, direction, boundingBox, position, res, obj) && res.res <= 1;
    }
}
